...
```
npm install
npm test          # 129 passing, 1 skipped — see "Known constraints" below
npm run typecheck
npm run explore -- --component <path> --export <ExportName> [options]
```
...
- `examples/DebouncedSearch.html` — 18 states (12 transient) against a hand-written 6;
  the corpus's genuine presentation failure case, written up in full in
  `docs/m5-report-notes.md`.

`docs/m5-report-notes.md` has the full per-component judgement on report usefulness —
what's reviewable, what isn't, and specifically why.
...
- `docs/m5-report-notes.md` — this milestone's per-component report-usefulness judgement.

## What's not built (M6 and beyond)

`approve`/baseline-diff (M6, a stretch goal in the plan) is not implemented: there is no
mechanism yet to check in a reviewed graph and diff a later run against it. Everything
up to and including the JSON/HTML report (M0–M5) is complete and tested.

## Status

125 tests from M0–M4, plus 5 added in M5 (JSON determinism, HTML report contents ×3,
CLI end-to-end via the skipped/unskipped runner test) = 130 total, 129 passing, 1
correctly skipped outside a real `npm run explore` invocation. `npm run typecheck`
passes with strict TypeScript throughout.
